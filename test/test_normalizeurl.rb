...
class TestNormalizeurl < Minitest::Test
  def test_that_it_has_a_version_number
    refute_nil ::Normalizeurl2025::VERSION
  end

  def test_normalize_basic_url
    url = "https://EXAMPLE.COM/Path/"
    expected = "https://example.com/Path"
    assert_equal expected, Normalizeurl2025.normalize(url)
  end
  ...
  def test_normalize_with_custom_options
    ...
    assert_equal expected, Normalizeurl2025.normalize(url, options)
  end

  def test_normalize_handles_nil_and_empty_urls
    assert_nil Normalizeurl2025.normalize(nil)
    assert_nil Normalizeurl2025.normalize("")
    assert_nil Normalizeurl2025.normalize("   ")
  end

  def test_normalize_handles_invalid_urls
    invalid_url = "not-a-url"
    assert_equal invalid_url, Normalizeurl2025.normalize(invalid_url)
  end

  def test_normalize_only_processes_http_urls
    ftp_url = "ftp://example.com/file.txt"
    assert_equal ftp_url, Normalizeurl2025.normalize(ftp_url)
  end
